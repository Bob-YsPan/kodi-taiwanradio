...
def get_m3u8_url(station_id):
    try:
        ...
        xbmc.log("%s Error: %s" % (station_id, e), xbmc.LOGERROR)
        return None
...
def play_station(station_id):
    ...
    if url:
        li_play = xbmcgui.ListItem("播放 " + STATIONS[station_id]['name'])
        ...
        li_refresh = xbmcgui.ListItem("重新整理串流")
        ...
        li_error = xbmcgui.ListItem("連線失敗")
